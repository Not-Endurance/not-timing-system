...
public class NDropdownButtonBehind : NComponent
{
    int _selectedIndex;

    protected NDropdownButtonDescriptor? SelectedDescriptor =>
        Descriptors.Count == 0 || _selectedIndex < 0 || _selectedIndex >= Descriptors.Count
            ? null
            : Descriptors[_selectedIndex];

    [Parameter]
    public IReadOnlyList<NDropdownButtonDescriptor> Descriptors { get; set; } = [];
...
public class NDropdownButtonDescriptor
{
    public NDropdownButtonDescriptor(string content, Func<Task> safeAction, string? icon = null)
    {
        Content = content;
