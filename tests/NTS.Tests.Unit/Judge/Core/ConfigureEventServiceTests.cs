...
using NTS.Judge.Features.Setup.ConfigureEvents;
...
public class ConfigureEventServiceTests
{
    [Fact]
    public async Task Validate_WhenConfigureEventIdTargetsInvalidEvent_ReturnsIssuesForThatEvent()
    {
    ...
    [Fact]
    public async Task Validate_WhenConfigureEventIdMissing_ThrowsGuardException()
    {
    ...
    [Fact]
    public async Task DeleteParticipation_WhenConfigureEventIdIsProvided_DeletesFromThatEventOnly()
    {
        var repository = new RecordingRepository<ConfigureEvent>(
            [
                CreateValidEvent(1, competitionId: 1, participationNumber: 101),
                CreateValidEvent(2, competitionId: 1, participationNumber: 101),
            ]
        );
        var service = new ConfigureEventService(repository, new TestNotifier());
    ...
    static ConfigureEventService CreateService(IEnumerable<ConfigureEvent> configureEvents)
    {
        return new ConfigureEventService(new RecordingRepository<ConfigureEvent>(configureEvents), new TestNotifier());
    }
    ...
    static ConfigureEvent CreateValidEvent(int id, int? competitionId = null, int? participationNumber = null)
    {
    ...
    static ConfigureEvent CreateInvalidEvent(int id)
    {
        var country = CreateCountry();
        var athlete = new Athlete(new Person(["John", "Doe"]), null, country, null, id * 10 + 1);
        var horse = new Horse($"Horse {id}", null, id * 10 + 1);
