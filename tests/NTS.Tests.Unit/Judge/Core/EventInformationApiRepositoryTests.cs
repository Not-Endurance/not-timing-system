...
public class EventInformationApiRepositoryTests
{
    [Fact]
    public async Task ReadActive_calls_active_endpoint_and_maps_events()
    {
        var models = new[] { EventInformationModel.From(CreateEvent(14)), EventInformationModel.From(CreateEvent(21)) };
        var handler = new RecordingHttpMessageHandler
        {
            ResponseFactory = _ => CreateJsonResponse(Result.Success<IEnumerable<EventInformationModel>>(models)),
        };
        ...
        Assert.Equal("https://nexus.test/api/event-information/active", request.RequestUri?.ToString());
    }

    [Fact]
    public async Task ReadPast_calls_past_endpoint_and_maps_events()
    {
        var models = new[] { EventInformationModel.From(CreateEvent(31)), EventInformationModel.From(CreateEvent(32)) };
        var handler = new RecordingHttpMessageHandler
        {
            ResponseFactory = _ => CreateJsonResponse(Result.Success<IEnumerable<EventInformationModel>>(models)),
        };
        ...
        var request = Assert.Single(handler.Requests);
        Assert.Equal(HttpMethod.Get, request.Method);
        Assert.Equal("https://nexus.test/api/event-information/past", request.RequestUri?.ToString());
    }

    [Fact]
    public async Task Start_returns_mapped_endurance_event_when_response_contains_success_result()
    {
        var model = EventInformationModel.From(CreateEvent(14));
        var handler = new RecordingHttpMessageHandler
        {
        ...
        Assert.Equal("https://nexus.test/api/event-information/14/start", request.RequestUri?.ToString());
    }
    ...
    public async Task Start_throws_domain_exception_when_response_contains_errors()
    {
        var handler = new RecordingHttpMessageHandler
        {
            ResponseFactory = _ => CreateJsonResponse(Result.Failure<EventInformationModel>("Start blocked")),
        };
        var client = CreateClient(handler);
        var repository = new EventInformationApiRepository(client, new TestSocketContext());
    ...
    public async Task Reset_calls_endurance_event_reset_endpoint_for_current_event()
    {
        ...
        Assert.Equal("https://nexus.test/api/event-information/14/reset", request.RequestUri?.ToString());
    }
